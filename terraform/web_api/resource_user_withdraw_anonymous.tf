...
module "user_withdraw_anonymous_post_lambda_function" {
  ...
  handler       = "user_withdraw_anonymous_user"
...
# Method
##################################################
module "user_withdraw_anonymous_post_method" {
  source      = "./method"
  rest_api_id = aws_api_gateway_rest_api.charalarm_rest_api.id
  resource_id = aws_api_gateway_resource.user_withdraw_anonymous_resource.id
  http_method = "POST"
  lambda_uri  = module.user_withdraw_anonymous_post_lambda_function.invoke_arn
}
